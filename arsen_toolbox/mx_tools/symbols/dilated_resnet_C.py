...
def drn_c(units, num_stages, filter_list, num_classes, image_shape, bottle_neck=True, bn_mom=0.9, workspace=256, memonger=False):
    """Return symbol of dilated resnet-C
    Parameters
    ----------
    units : list
    ...
    for i in range(2, 4):
        body = drn_unit(body, filter_list[i+1], (1 if i==2 else 2, ) * 2, (1, 1), False,
                             ...
                             memonger=memonger)
        for j in range(units[i]-1):
            body = drn_unit(body, filter_list[i+1], stage_dilated[i==3], (1, 1), True, 
                                name='stage%d_unit%d' % (i + 1, j + 2), bottle_neck=bottle_neck, 
                                workspace=workspace, memonger=memonger)

    # stages: conv5 - conv6, using resnet unit without shortcut
    for i in range(4, 6):
        body = stacked_conv_unit(body, filter_list[-1], (1, 1), name='stage%d_unit%d' % (i + 1, 1), 
                                bottle_neck=bottle_neck, workspace=workspace, memonger=memonger)
